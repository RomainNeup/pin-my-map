<script lang="ts">
	...
	onMount(() => {
		...
		if (urlTagIds) selectedTagIds = urlTagIds.split(',').filter(Boolean);
	...
	function syncUrl(tagIds: string[], done: DoneFilterValue) {
		...
		else url.searchParams.delete('tagIds');
		if (done !== 'all') url.searchParams.set('done', done);
	...
	async function fetchPage(reset: boolean) {
		...
		try {
			const res = await getSavedPlaces({ limit: PAGE_SIZE, offset, tagIds: selectedTagIds });
...
<div class="w-full">
	...
	{:else if sorted.length === 0}
		{#if selectedTagIds.length > 0 || doneFilter !== 'all'}
			<EmptyState
				title="No matches"
	...
	{:else}
		...
		{#if hasMore && !loading}
			<div class="mt-6 flex justify-center">
				<Button variant="outline" tone="neutral" onclick={() => fetchPage(false)}>Load more</Button>
			</div>
		{/if}
	{/if}
</div>
